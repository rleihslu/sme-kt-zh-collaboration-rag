...
The `evaluation` module measures the quality of the full RAG pipeline along two orthogonal axes: **retrieval quality** (reference-based, requires a labelled dataset) and **generation quality** (LLM-as-judge, no ground truth required).

#### Offline batch evaluation

Run against a pre-built dataset of queries and known-relevant chunk IDs:
...
samples = [
    EvaluationSample(
        ...
        retrieved_chunks=[...],          # list[ChunkRecord] or list[ChunkMatch]
...
#### Online diagnostic mode — build samples from a live agent

Run the full pipeline on a query set and measure the result automatically:
...
samples = asyncio.run(
    Evaluator.build_samples_from_agent(
        ...
        relevant_chunk_ids=[{"chunk-2"}, {"chunk-1", "chunk-4"}],
    )
)
report = asyncio.run(Evaluator([HitRate(5), Faithfulness(judge)]).evaluate(samples))
```

#### Retrieval-only mode

Isolate retrieval quality without running the LLM generator:
...
report = asyncio.run(Evaluator([HitRate(5), MRR(5), RecallAtK(5)]).evaluate(samples))
```
...
|---|---|---|
| `HitRate(k)` | Retrieval | Fraction of queries with at least one relevant chunk in top-k |
| `MRR(k)` | Retrieval | Mean reciprocal rank of the first relevant chunk |
| `PrecisionAtK(k)` | Retrieval | Fraction of top-k results that are relevant |
| `RecallAtK(k)` | Retrieval | Fraction of all relevant chunks found in top-k |
| `NDCGAtK(k)` | Retrieval | Normalised discounted cumulative gain (log2 discount) |
| `Faithfulness(llm)` | Generation | Is the answer grounded in the retrieved context? |
| `AnswerRelevance(llm)` | Generation | Does the answer directly address the query? |
| `ContextRelevance(llm)` | Generation | Is the retrieved context useful for the query? |

Retrieval metrics require `relevant_chunk_ids` on each sample. Generation metrics use an LLM judge that emits a 0-5 score normalised to [0, 1]. Any judge failure logs a warning and contributes 0.0 — the pipeline never raises.

#### RAGAS adapter (optional)

If you have RAGAS installed (`pip install 'conversational-toolkit[eval-ragas]'`), you can convert samples to a RAGAS dataset or run RAGAS metrics and get a toolkit `EvaluationReport` back:

```python
from conversational_toolkit.evaluation.adapters import to_ragas_dataset, evaluate_with_ragas
